...
Информация о клиенте
 ...
 ◉ Телефон: {order.client_info.phone}
    '''
    keyboard = get_in_place_keyboard(order.pk, staff.staff.pk)
        ...
        if staff.is_accept == False and staff.is_refuse == False:
            try:
                with transaction.atomic():
                    ...
                    report = ManagerReport.objects.create(cleaner=staff.staff, order=order, fine=200,
                                                          comment=fine_comment, salary=0)
                ...
                text = "Повроторите запрос позже"
                context.bot.send_message(chat_id, text)
        else:
            text = "Даный запрос не может быть выполнен"
            context.bot.edit_message_text(chat_id=chat_id, message_id=message_id, text=text)
